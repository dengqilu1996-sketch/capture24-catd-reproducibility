"""Train a compact 10-class multi-head CNN and evaluate CATD on its posteriors.

This is a supplementary, current-task experiment.  Fine, coarse, and MET-intensity
heads share a 1-D convolutional trunk and are trained on P001--P080.  P081--P100
...
OUT=ROOT/"results/deep_catd_20260904"
...
class WindowDataset(Dataset):
    ...
    def __getitem__(self,j):
        i=int(self.idx[j]); x=torch.from_numpy(np.asarray(self.X[i],dtype=np.float32).T)
...
class MultiHeadCNN(nn.Module):
    def __init__(self,nf,nc,ni):
        ...
        self.trunk=nn.Sequential(
            nn.Conv1d(3,32,7,2,3,bias=False),nn.BatchNorm1d(32),nn.ReLU(inplace=True),
            nn.Conv1d(32,64,5,2,2,bias=False),nn.BatchNorm1d(64),nn.ReLU(inplace=True),
            nn.Conv1d(64,128,5,2,2,bias=False),nn.BatchNorm1d(128),nn.ReLU(inplace=True),
            nn.Conv1d(128,256,3,2,1,bias=False),nn.BatchNorm1d(256),nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool1d(1))
        self.shared=nn.Sequential(nn.Flatten(),nn.Linear(256,128),nn.ReLU(inplace=True),nn.Dropout(.2))
        self.fine=nn.Linear(128,nf); self.coarse=nn.Linear(128,nc); self.intensity=nn.Linear(128,ni)
    def forward(self,x):
        h=self.shared(self.trunk(x)); return self.fine(h),self.coarse(h),self.intensity(h)
...
def main():
    ...
    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'); model=MultiHeadCNN(len(FINE),len(COARSE),len(INTENS)).to(device)
    ...
    rows=[]
    for name,pred in [('cnn',FINE[ np.argmax(testres['logits'][0],axis=1)]),('cnn_hmm',predh),('cnn_temporal_only',predt),('cnn_catd',predc)]: rows.append({'method':name,'alpha':float(sel.alpha) if name=='cnn_catd' else (0 if name=='cnn_temporal_only' else np.nan),'beta':float(sel.beta) if name=='cnn_catd' else (0 if name=='cnn_temporal_only' else np.nan),'gamma':float(sel.gamma) if name=='cnn_catd' else (float(to.gamma) if name=='cnn_temporal_only' else (1 if name=='cnn_hmm' else np.nan)),'macro_f1':macro(yf0[si],pred),'balanced_accuracy':metrics.balanced_accuracy_score(yf0[si],pred)})
