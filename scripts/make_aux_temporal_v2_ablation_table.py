...
def load_xgb_rows(path: Path) -> list[dict]:
    """Load locally computed results; no observed metrics are bundled."""
    df = pd.read_csv(path)
    required = {"family", "method", "selection_role", "alpha", "beta",
                "gamma_or_gate", "macro_f1", "balanced_accuracy",
                "intensity_conflict", "any_conflict", "corrected", "harmed"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Local XGBoost CSV is missing columns: {sorted(missing)}")
    if len(df) != 2 or set(df["method"]) != {"XGB baseline", "post-hoc BCM"}:
        raise ValueError("Provide exactly one XGB baseline and one post-hoc BCM row.")
    if not (df["family"] == "XGBoost").all():
        raise ValueError("Expected XGBoost rows only.")
    for column in ("macro_f1", "balanced_accuracy", "intensity_conflict", "any_conflict"):
    ...
    for column in ("corrected", "harmed"):
        values = pd.to_numeric(df[column], errors="raise")
        if not np.isfinite(values).all() or (values < 0).any() or (values % 1 != 0).any():
            raise ValueError(f"{column} must contain non-negative integer counts.")
    return df[sorted(required)].to_dict(orient="records")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build a table from local experiment outputs.")
    parser.add_argument("--xgb-rows", type=Path, required=True,
                        help="Local two-row XGBoost result CSV; not distributed.")
    ...
    rows.extend(load_xgb_rows(args.xgb_rows))
    ...
    md = [
        ...
        "",
        "Interpret results from the supplied local inputs; no fixed experimental conclusions are bundled.",
    ]
